...
const Login = () => {
  ...
  const onSubmit = (e) => {
    ...
    axios.post("https://uzum-api.onrender.com/api/auth/login", { email, password })
      .then((result) => {
        if (result.data.success && result.data.data) {
          const user = result.data.data;
          localStorage.setItem("accessToken", user.accessToken || "");
          localStorage.setItem("user", JSON.stringify(user));
          toast.success("Login muvaffaqiyatli!", { position: "top-right", autoClose: 1000, theme: "colored" });
          setTimeout(() => navigate("/"), 1000);
        } else {
          toast.error(result.data.message || "Login yoki parol xato!", { position: "top-right", autoClose: 3000, theme: "colored" });
        }
      })
  ...
  return (
    <div className="login-container">
      ...
      <form className="login-card" onSubmit={onSubmit}>
        <div className="lock-icon-wrapper"><i className="fa-solid fa-lock lock-icon"></i></div>
